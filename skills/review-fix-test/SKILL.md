...
description: Single subagent-backed git review workflow that reviews, fixes, verifies, and optionally commits current changes, the latest commit, or both. Use when invoked as `$review-fix-test` or through Korean shortcuts such as "변경리뷰", "커밋리뷰", "동시리뷰", "반복변경리뷰", "반복커밋리뷰", or similar. Supports target=changes|commit|both, positional shorthands like `commit 5`, max review cycles, and verify_max validation retries.
---
...
1. Inspect repository state: current branch, `git status --short`, and the selected target diff.
2. Spawn a fresh `explorer` agent to review the selected target. The explorer must not edit files.
