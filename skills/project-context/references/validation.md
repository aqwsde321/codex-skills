...
candidate validation 실패 시 generated index, metadata, plan을 실행 전 상태로 복구한다. 문제를 수정하고 같은 snapshot 기준으로 finalize를 다시 실행한다.

최종 on-disk validation 실패 시 helper가 generated index, 이전 metadata, 삭제한 plan을 모두 실행 전 상태로 복구한다. 문제를 수정한 뒤 다시 finalize한다.
