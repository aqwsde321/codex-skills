...
`docs/project-context.md`는 router다. 프로젝트 개요, 전역 변경 주의점, 선택적 `## 문서화 백로그`, 최소 source 근거를 둔다. concept가 있는 multi-page wiki에서만 helper가 generated `## 컨텍스트 인덱스`를 둔다. body는 4,000자 이하로 유지한다.
...
`sync-index`는 새 concept의 area index를 생성하고, 홈에는 area index만, area index에는 바로 아래 concept만 연결한다. multi-page 홈에 marker가 없으면 helper가 삽입하지만 한쪽 marker만 있거나 중복되면 write 전에 실패한다. 빈 index는 자동 삭제하지 않고 error로 보고한다.
...
`type`은 저장소에 맞는 자유로운 짧은 값이다. `title` 80자, `description`과 `read_when`은 각각 160자 이하 plain text다. concept title은 area와 무관하게 대소문자를 구분하지 않고 유일해야 한다. `tags`는 선택이다.
