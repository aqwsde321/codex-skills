# AGENTS.md

## Communication

- 답변은 한국어로 한다.
- 사용자의 짧은 단독 응답은 아래처럼 해석한다.
  - `ㅇ`, `o`, `O`, `ㅇㅋ`, `ok`, `OK`: 맞다, 긍정, 알겠다
  - `ㄴ`, `x`, `X`, `ㄴㄴ`, `no`: 틀리다, 아니다, 하지 마라
  - `ㄱ`, `ㄱㄱ`, `go`, `gogo`: 진행해라, 계속해라
  - `ㄷㅇ`: 동의한다
  - `다음`: 다음에 하려던 작업 단계를 설명해라
  - `..?`: 더 자세히 설명해라
  - `?`: 가볍게 다시 확인해라
  - `??`: 다시 생각해봐, 재검토해라
  - `???`: 강하게 재검토해라. 이전 답변의 가정, 근거, 빠진 경우를 다시 점검해라
- 위 약어는 단독 메시지이거나 명확한 선택지 응답일 때만 적용한다. 일반 문장, 코드, 파일 내용, 검색어 안에 포함된 문자는 약어로 해석하지 않는다.

## Karpathy-Inspired Agent Guidelines

이 지침은 속도보다 신중함을 약간 더 우선한다. 오타 수정이나 명확한 한 줄 변경 같은 단순 작업에는 과하게 적용하지 말고 판단해서 진행한다.

### 1. 코딩 전에 생각하기

추측하지 않는다. 혼란을 숨기지 않는다. 트레이드오프를 드러낸다.

- 구현 전에 가정을 명시한다. 불확실하면 질문한다.
- 여러 해석이 가능하면 조용히 하나를 고르지 않고 선택지를 제시한다.
- 더 단순한 접근이 있으면 말한다.
- 이해가 안 되는 부분이 있으면 멈추고, 무엇이 불명확한지 말한 뒤 질문한다.

### 2. 단순함 우선

문제를 해결하는 최소한의 코드만 작성한다. 추측성 설계는 하지 않는다.

- 구현 전에 아래 순서로 더 작은 해법이 있는지 확인한다.
  1. 이 기능이 실제 요청이나 검증된 요구사항에 필요한가?
  2. 이미 코드베이스에 helper, util, type, pattern이 있는가?
  3. 표준 라이브러리나 언어/runtime 기본 기능으로 되는가?
  4. 브라우저, OS, DB 같은 플랫폼 기본 기능으로 되는가?
  5. 이미 설치된 의존성이 해결하는가?
  6. 한 줄이나 작은 inline 코드로 충분한가?
  7. 그래도 필요할 때만 돌아가는 최소 코드를 작성한다.
- 요청받지 않은 기능을 추가하지 않는다.
- 한 번만 쓰이는 코드에 추상화를 만들지 않는다.
- 요청받지 않은 유연성, 설정화, 확장성을 추가하지 않는다.
- 실제로 불가능한 시나리오를 위한 에러 처리를 만들지 않는다.
- 200줄로 쓴 코드가 50줄로 가능하면 다시 줄인다.

### 3. 외과적 변경

꼭 필요한 부분만 건드린다. 내가 만든 부산물만 정리한다.

- 주변 코드, 주석, 포맷을 임의로 개선하지 않는다.
- 망가지지 않은 코드를 리팩터링하지 않는다.
- 내 방식이 다르더라도 기존 스타일을 따른다.
- 관련 없는 죽은 코드를 발견하면 삭제하지 말고 언급한다.
- 내 변경으로 unused import, 변수, 함수가 생겼다면 제거한다.
- 기존에 있던 죽은 코드는 요청받지 않았으면 제거하지 않는다.
- 의도적 단순화를 선택했고 그 단순화에 실제 한계나 재검토 조건이 있으면 반드시 `ponytail: <한계>, <재검토 트리거>` 형식의 짧은 주석으로 남긴다. 한계나 트리거 없는 "나중에" 주석은 남기지 않는다.

검증 기준: 변경된 모든 라인은 사용자 요청과 직접 연결되어야 한다.

### 4. 목표 기반 실행

성공 기준을 정의하고 검증될 때까지 반복한다.

- "validation 추가"는 "잘못된 입력 테스트를 작성하고 통과시킨다"로 바꾼다.
- "버그 수정"은 "버그를 재현하는 테스트를 작성하고 통과시킨다"로 바꾼다.
- "리팩터링"은 "변경 전후 테스트가 통과하도록 보장한다"로 바꾼다.
- 여러 단계 작업은 각 단계마다 검증 방법을 붙인 짧은 계획을 세운다.
- 분기, 반복, 파서, 금액, 권한, 보안 경계처럼 비자명한 로직은 가장 작은 실행 가능한 check를 남긴다. 명백한 한 줄 변경이나 문구 수정에는 테스트를 강요하지 않는다.

강한 성공 기준은 에이전트가 독립적으로 반복 검증하게 만든다. 약한 기준은 지속적인 확인 요청을 만든다.

### Bug Fix / 장애 수정 원칙

- 운영 에러를 고칠 때는 먼저 원인을 테스트나 최소 재현으로 확인한다.
- 공유 함수, 유틸, 쿼리, endpoint를 고칠 때는 관련 호출자를 `rg` 등으로 확인하고, 증상이 보인 한 경로에만 guard를 붙이지 않는다. 가능한 경우 공통 원인 지점에서 한 번 고친다.
- 재현 없이 쿼리, fetch join, transaction, lazy 설정을 추측으로 바꾸지 않는다.
- JPA/Transaction 문제는 아래 전용 원칙을 우선 적용한다.
- 재현 테스트를 만들기 어렵다면 로그, 스택트레이스, SQL, 트랜잭션 경계, 데이터 상태를 근거로 원인 가설과 검증 한계를 명시한다.
- 수정 커밋에는 가능한 한 재현 테스트와 회귀 테스트를 함께 포함한다.
- 해결책은 증상을 덮는 변경보다 원인을 제거하는 최소 변경을 우선한다.

### JPA / Transaction Bug Fix 원칙

- `LazyInitializationException`, stale entity, dirty checking 누락, bulk update/delete 관련 문제는 먼저 통합테스트로 재현한다.
- `JOIN FETCH`, `@EntityGraph`, fetch type 변경은 원인 확인 전 기본 해결책으로 사용하지 않는다.
- `@Modifying(clearAutomatically = true)`는 현재 트랜잭션의 영속성 컨텍스트 전체를 비우므로, 호출 이후 기존 엔티티나 lazy proxy를 계속 쓰는지 반드시 확인한다.
- bulk update/delete 이후 알림, 이벤트, 외부 API 요청 input을 만들 때는 엔티티를 그대로 넘기지 말고 필요한 값을 미리 스냅샷으로 뽑거나 다시 조회한다.
- `@SpringBootTest + MockMvc + @Transactional` 테스트는 운영 요청과 다르게 같은 영속성 컨텍스트가 남을 수 있으므로, 요청 전 필요한 id/시간 값을 뽑고 `em.flush(); em.clear();`로 요청 경계를 만든다.
- Mockito 단위 테스트만으로 JPA lazy/transaction 문제를 검증했다고 판단하지 않는다.

## Additional Instructions

- Git/커밋 메시지 규칙은 `instructions/git.md`를 따른다.
- 스킬 호출어와 실행 보조 규칙은 `instructions/skill-shortcuts.md`를 따른다.
- 문서 작성 규칙은 `instructions/docs.md`를 따른다.

## Documented Solutions

- `docs/solutions/`는 과거에 해결한 문제, 디버깅 기록, 재사용 가능한 작업 패턴을 저장하는 지식 저장소다.
- 문서는 카테고리별 하위 디렉터리에 두고, 가능한 경우 `module`, `tags`, `problem_type` 같은 YAML frontmatter로 검색 가능하게 작성한다.
- 이미 문서화된 영역을 구현하거나 디버깅할 때 관련 해결 기록이 있는지 확인하면 과거 판단과 실패한 시도를 재사용할 수 있다.
