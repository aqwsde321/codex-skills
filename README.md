...
| `40리뷰` | `$review-fix-test` | 작업 후 변경분이나 커밋을 리뷰, 수정, 검증할 때 |
...
| `review-fix-test` | `40리뷰`, `변경리뷰`, `커밋리뷰`, `동시리뷰`, `반복리뷰`, `반복변경리뷰`, `반복커밋리뷰` | 현재 변경분, 마지막 커밋, 또는 둘 다를 Core Risk, Standards, Spec 축으로 리뷰/수정/검증한다. |
